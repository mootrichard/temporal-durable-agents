# Frame packet: 06-replay-resumes
...
- duration: 8.469s
- poster: 6.2s
- transition_in: replay-scan
- type: feature_showcase
- blueprint: agent-progress-theater (Reproduce)
- scene: Event History receipts cascade into a replacement Worker; completed branches lock green, an interrupted Activity resumes from heartbeat details, and passed test files are skipped.
- voiceover: "A replacement Worker replayed Event History, reused results, resumed heartbeated work, and skipped tests already passed."
...
Scene 1 (0.0–1.5s): Event History is already open at left; a replacement Worker card triggers at right with `ATTEMPT 2`, using the approved split layout and a locked camera.
Scene 2 (1.5–3.4s): recorded plan and completed Child Workflow rows cascade into the receipt ledger (`spring-pop-entrance` stagger); each flips to a green check as “reused recorded results” is spoken.
Scene 3 (3.4–5.7s): `HEARTBEAT · THREAD ID` and `TEST CHECKPOINT · 2/4` expand from the interrupted Activity row (`anchored-layout-expand`), then route into the Worker.
Scene 4 (5.7–7.1s): the Worker performs: thread becomes `RESUMED`, two passed test filenames strike through and dim, and remaining files advance; finite spinner motion stops the instant each receipt resolves.
Scene 5 (7.1–8.469s): final receipt card fills the right panel: `REPLAY COMPLETED WORK · RETRY UNFINISHED WORK`; the same Workflow ID stays visible and the frame holds.
