# Frame packet: 03-process-memory-fails
...
- scene: A clean split contrasts active JavaScript promises with the captured Baseline failure state; the continuation column empties when the process dies.
- voiceover: "When process memory owns the continuation, process death erases what happens next. Restart means zero."
...
Adapt: keep the mirrored split-tilt entry; compare the cause (`process memory owns continuation`) with its captured consequence (`restart from zero`) rather than two equal product features.

Scene 1 (0.0–1.4s): `WHO OWNS WHAT HAPPENS NEXT?` slides down into the upper third on a locked camera; the answer is withheld.
Scene 2 (1.4–3.5s): two equal cards enter from opposite wings with mirrored book-open tilts (`split-tilt-cards`): left shows JavaScript promises, thread mapping, and next step; right holds the real Baseline failure capture.
Scene 3 (3.5–5.4s): inner-edge badges punctuate on the VO cues: `PROCESS MEMORY` on the left, `WORKERS STOPPED` on the right; the screenshot region magnifies only enough to read “Process memory was lost.”
Scene 4 (5.4–6.5s): the left continuation rows hard-swap to empty outlines while the right failure state holds; a single error line draws through the shared center.
Scene 5 (6.5–7.552s): both cards settle flat and a violet bottom rail lands: `RESTART = START FROM ZERO`; camera and cards hold dead still.
