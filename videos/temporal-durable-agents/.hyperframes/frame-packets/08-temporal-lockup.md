# Frame packet: 08-temporal-lockup
...
- duration: 9.301s
- poster: 4.3s
- transition_in: clean-cut
- type: branding
- blueprint: logo-assemble-lockup (Adapt)
- scene: The execution tree collapses into the official Temporal wordmark; a final two-line principle resolves beneath it.
- voiceover: "Temporal owns orchestration recovery. Applications own safe external effects. The Worker executes the work. It does not own it."
...
- roles: .media/images/logo_001.svg = hero official brand lockup · .media/images/image_001.png = supporting proof fragments
...
Adapt: keep the parts-clear → official lockup → long-held principle arc; the execution-tree geometry collapses into the brand anchor, but the official wordmark itself is only revealed, never redrawn or altered.

Scene 1 (0.0–1.3s): small proof fragments—Workflow ID, 4/4, one-line diff—collapse inward from the approved tree layout (`center-outward-expansion` run in reverse), clearing the stage to the dark brand field.
Scene 2 (1.3–2.7s): the official Temporal wordmark reveals left-to-right from a single mask; a violet underline self-draws beneath it (`svg-path-draw`) and stops.
Scene 3 (2.7–4.2s): `THE WORKER EXECUTES THE WORK` assembles chunk-by-chunk below the lockup via dynamic content sequencing; Event History and application-boundary labels seat at the lower rule.
Scene 4 (4.2–9.301s): `IT DOES NOT OWN IT` lands in violet at three-to-one scale contrast; the lockup, principle, and small footer hold completely still through the final frame.
