...
const [sourceTurn, testTurn, initialTests] = await Promise.all([
  this.runInvestigation(
    input, sourceAssignment, 'source-investigator', emit,
  ),
  this.runInvestigation(
    input, testAssignment, 'test-investigator', emit,
  ),
...
const initialTestsPromise = activities.runTests({
  workspace: input.workspace, phase: 'initial',
});
```
...
export async function SubagentWorkflow(input: SubagentWorkflowInput) {
  // !focus(1:7)
  const role = input.assignment.focus === 'source'
    ? 'source-investigator'
    : 'test-investigator';
...
return executeCodex(
  runner,
  { ...input, threadId: durableThreadId },
  replacementThread,
);
